...
export class CoreService {
  ...
  async preflight(input: { phone_e164: string; provider_call_id: string; asterisk_uniqueid: string; started_at: string }): Promise<any> {
    ...
    const denied = await this.preflightDenyReason(account);
  ...
  private async preflightDenyReason(account: AccountEntity): Promise<DenyPrompt | null> {
    ...
    const lockSet = await this.redis.acquireActiveCall(account.phone_e164);
    if (!lockSet) {
      ...
      if (!(await this.redis.acquireActiveCall(account.phone_e164))) return 'active_call_exists';
    }
    return null;
  }
  ...
  async endCall(payload: { call_session_id: string; phone_e164: string; ended_reason: CallEndedReason; ended_at: string }): Promise<any> {
    ...
    await this.redis.releaseActiveCall(session.phone_e164);
  ...
  async bridgeEnded(): Promise<{ ok: boolean }> {
    return { ok: true };
  }
  ...
  async adminSetStatus(phone: string, status: 'active' | 'blocked', identity: string): Promise<{ ok: boolean }> {
    const acc = await this.ensureCaller(phone);
    const before = { ...acc };
    acc.status = status;
    await this.accounts.save(acc);
    await this.audits.save(this.audits.create({ admin_identity: identity, action_type: status === 'blocked' ? 'block_account' : 'unblock_account', target_phone_e164: phone, before_json: before, after_json: acc as any }));
    return { ok: true };
  }

  async adminAdjust(phone: string, seconds: number, kind: 'credit' | 'debit', identity: string): Promise<{ ok: boolean; remaining_seconds: number }> {
    const remaining = await this.ds.transaction(async (trx) => {
      ...
      await trx.getRepository(AccountEntity).save(acct);
      await trx.getRepository(BalanceLedgerEntity).save(trx.getRepository(BalanceLedgerEntity).create({
        phone_e164: phone, entry_type: kind === 'credit' ? 'admin_credit' : 'admin_debit', delta_seconds: delta, reference_type: 'admin', reference_id: identity, metadata_json: {},
      }));
      await trx.getRepository(AdminAuditLogEntity).save(trx.getRepository(AdminAuditLogEntity).create({
        admin_identity: identity, action_type: `admin_${kind}`, target_phone_e164: phone, before_json: before as any, after_json: acct as any,
      }));
      return acct.remaining_seconds;
    });
  ...
  async adminTerminate(callSessionId: string, identity: string): Promise<{ ok: boolean }> {
    await this.createForceEnd(callSessionId, 'backend_revoke');
    await this.audits.save(this.audits.create({ admin_identity: identity, action_type: 'terminate_call', target_phone_e164: null, before_json: null, after_json: { call_session_id: callSessionId } }));
    return { ok: true };
  }
  ...
  async adminListAccounts(search?: string, status?: string, page = 1): Promise<any> {
    ...
    qb.orderBy('a.created_at', 'DESC').skip((page - 1) * 50).take(50);
    return { items: await qb.getMany(), page };
  }

  async adminGetAccount(phone: string): Promise<any> {
    const account = await this.accounts.findOneByOrFail({ phone_e164: phone });
    const ledger = await this.ledger.find({ where: { phone_e164: phone }, order: { created_at: 'DESC' }, take: 100 });
